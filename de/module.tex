...
\ \\ \ \\
Mit dem Befehl \texttt{{\textbackslash}ifttm} kann abgefragt werden, ob eine PDF- oder HTML-Konvertierung erfolgt. In diesem
Fall ist
\ifttm%
ein HTML-Modul
\else%
...
mit dem \texttt{{\textbackslash}texttt}-Befehl in einer Mathematikumgebung anfangen:
$$
...
\ \\ \ \\
Man kann aber auch Links \MExtLink{http://www.mint-kolleg.de/}{aus dem Kurs heraus} setzen, natürlich auf eigene Gefahr.
\end{MXContent}
...
\begin{MXContent}{Strukturierung}{Strukturierung}{STD}
    ...
    \item{\texttt{{\textbackslash}}\verb$begin{MSectionStart} ... \end{MSectionStart}$} % hier muss verhindert werden, dass der Konverter MSectionStart erkennt und übersetzt
    \item{\verb$\MSubsection{UNTERTITEL}$}
    \item{\texttt{{\textbackslash}}\verb$begin{MXContent}{TITEL}{KURZTITEL}{TYP}$ (Abschnittstyp in der Regel \texttt{STD})}
    ...
    \begin{MExample}
    Das Wort \MEntry{Schlagwort}{Schlagwörter} wird unter \texttt{Schlagwörter} in der Liste aufgeführt mit dem Befehl \texttt{{\textbackslash}}\verb$MEntry{Schlagwort}{Schlagwörter}$.
...
Erstellt eine DropDown-Liste, in der Auswahlpunkte über den Befehl \texttt{{\textbackslash}MDropDownItem\{TEXT\}\{TAG\}} angeordnet werden, jeder mit einem Tag versehen.
...
diese werden dann durch einen mit \texttt{{\textbackslash}MGroupButton\{BUTTONTITEL\}} erzeugten Kontrollbutton ausgewertet.
\end{MInfo}
...
\begin{MLDropDownQuestion}{EXMLDROPDOWN1}{lsg}
...
\begin{MLDragDropQuestion}{MASETS}{r6:6;r3:3;r4:4}
...
\ \\ \ \\
Das Fragefeld wurde mit dem Kommando \texttt{{\textbackslash}MLQuestion\{20\}\{wachsend,steigend\}\{EX\_MLQuestion1\}} erzeugt.
\end{MExample}
...
% TODO: fix MLQuestionArea
%\MLQuestionArea{40}{}{3}{100}{EX_MLQuestionArea_1}
\ \\
Das Fragefeld wurde mit dem Kommando \texttt{{\textbackslash}MLQuestionArea\{40\}\{\}\{3\}\{100\}\{EX\_MLQuestionArea\_1\}} erzeugt.
\end{MExample}
...
Das Fragefeld wurde mit dem Kommando \texttt{{\textbackslash}MLParsedQuestion\{3\}\{wurzel(36)\}\{5\}\{EX\_MLParsedQuestion1\}} erzeugt.}
...
Das Fragefeld wurde mit dem Kommando \texttt{{\textbackslash}MLParsedQuestion\{20\}\{2*9.81\}\{5\}\{EX\_MLParsedQuestion2\}} erzeugt, es akzeptiert sowohl die numerische
...
Das Fragefeld wurde mit dem Kommando \texttt{{\textbackslash}MLParsedQuestion\{20\}\{5/(cos(pi/3))\}\{25\}\{EX\_MLParsedQuestion3\}} erzeugt.
...
\ \\
\begin{itemize}
\item{\texttt{5/cos(1/3*pi)} oder auch \texttt{5/cos({\textbackslash}pi/3)} sind richtige Eingaben.}
...
\ \\ \ \\
Das Fragefeld wurde mit dem Kommando \texttt{{\textbackslash}MLParsedQuestion\{20\}\{0,3\}\{5\}\{EX\_MLParsedQuestion4\}} erzeugt.
\ \\
\begin{itemize}
...
\ \\ \ \\
Dieses Fragefeld wurde mit dem Befehl \texttt{{\textbackslash}MLIntervalQuestion\{20\}\{(1,sqrt(2)]\}\{5\}\{EX\_MIntervalQuestion1\}} erzeugt.
\end{MExample}
...
\ \\ \ \\
Das Fragefeld wurde mit dem Kommando \texttt{{\textbackslash}MLFunctionQuestion\{20\}\{6*x\^{}2+x-6\}\{5\}\{x\}\{5\}\{EX\_MLFunctionQuestion1\}} erzeugt. Bei der Eingabe erhält der Benutzer einen
...
\ \\ \ \\
Das Fragefeld wurde mit dem Kommando \texttt{{\textbackslash}MLSimplifyQuestion\{40\}\{(a+b)*(1+x)\}\{5\}\{a,b,x\}\{5\}\{EX\_MLSimplifyQuestion1\}} erzeugt.
...
\ \\ \ \ \\
Das Fragefeld wurde mit dem Kommando \texttt{{\textbackslash}MLSimplifyQuestion\{30\}\{prod\_(k=1)\^{}(100)a\^{}k\}\{5\}\{a\}\{5\}\{0\}\{EX\_MLSimplifyQuestion2\}} erzeugt.
 \ \\
\begin{itemize}
\item{Die Eingaben \texttt{prod\_(k=1)\^{}(100) a\^{}k} oder ähnliche Schreibweisen mit \texttt{produkt} oder \texttt{{\textbackslash}prod} werden akzeptiert, der Name der Laufvariablen ist frei wählbar (wobei sie nicht in der Variablenliste aufgeführt werden darf).}
...
\begin{MInfo}
Das Kommando \texttt{{\textbackslash}MLSpecialQuestion\{ZEICHENZAHL\}\{LÖSUNG\}\{STÜTZSTELLENZAHL\}\{VARIABLE\}\{GENAUIGKEIT\}\{TYP\}\{UXID\}}
...
\ \\ \ \\
Das Feld wurde mit dem Befehl \texttt{{\textbackslash}MLSpecialQuestion\{10\}\{(4,6)\}\{1\}\{\}\{5\}\{intervalelement\}\{EX\_MLSpecialQuestion1\}} erzeugt.
\end{MExample}
...
\ \\ \ \\
Das Feld wurde mit dem Befehl \texttt{{\textbackslash}MLSpecialQuestion\{10\}\{42/(-12)\}\{1\}\{\}\{5\}\{exactfraction\}\{EX\_MLSpecialQuestion2\}} erzeugt.
\ \\ \ \\
...
mithilfe des Kommandos \texttt{{\textbackslash}special\{html:\textit{HTML-Code}\}} (auch inline) oder mit der \texttt{html}-Umgebung.
\ \\ \ \\
Dabei sollte stets mit \texttt{{\textbackslash}ifttm} die HTML-Übersetzung abgefragt und im anderen Fall (PDF) ein Ersatztext angegeben werden.
\end{MInfo}

\begin{MExample}
\ifttm
Dieses \special{html:<div style="background-color:rgb(255,0,0);">gefärbte&nbsp;div</div>} erscheint im HTML mit Hilfe
des Befehl \texttt{{\textbackslash}special\{html:<div style="background-color:rgb(255,0,0);{"}>gefärbte\&nbsp;div</div>\}}.
...
\texttt{{\textbackslash}special\{html:<script srcx="{}$:$directmaterial$:$dateiname.js"{} type="{}text/javascript"{}></script>\}}
vornehmen. Dabei ist zu beachten, dass die JavaScript-Datei mit dem Befehl \texttt{{\textbackslash}MRegisterFile\{DATEINAME\}} registriert
...
\begin{MExample}
Mit dem Befehl \texttt{{\textbackslash}MPythonScript\{DATEINAME\}} kann ein Pythonskript ausgeführt werden, seine Ausgabe wird als LaTeX-Code interpretiert
...
Mit dem Befehl \texttt{{\textbackslash}MYoutubeVideo\{TITEL\}\{BREITE\}\{HÖHE\}\{URL\}} kann ein embedded-Youtubevideo eingebunden werden, dazu wird ein von YouTube erzeugter
Player auf der Seite eingeblendet, der bei Bedarf vergrößert werden kann.
\ \\
% TODO fix MYoutubeVideo
